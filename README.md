# 👋 Hi, I'm Leen Rajab

![Banner](https://capsule-render.vercel.app/api?type=waving\&color=0\:FF4B2B,100\:FF416C\&height=200\&section=header\&text=Leen%20Rajab\&fontSize=50\&animation=twinkling)

### 💻 Computer Science Student | Backend Developer

Passionate about building scalable backend systems and full-stack web applications using modern technologies.
Currently focusing on ASP.NET Core, REST APIs, SQL Server, and software engineering best practices.

![Profile Views](https://komarev.com/ghpvc/?username=leenmahdi\&label=Profile%20views\&color=0e75b6\&style=flat)

---

## 🚀 Tech Stack

### 🧠 Backend

* ⚙️ ASP.NET Core
* 💙 C#
* 🗄️ Entity Framework Core
* 🔐 JWT Authentication
* ⚡ SignalR
* 🌐 RESTful APIs

### 🎨 Frontend

* ⚛️ React.js
* 🎨 HTML5
* 🎨 CSS3
* ⚡ JavaScript

### 🗃️ Database

* 🟦 SQL Server
* 🐘 PostgreSQL

### 🛠️ Tools & Platforms

* 🧰 Git & GitHub
* 📮 Postman
* 💻 Visual Studio
* 🖥️ VS Code
* 📚 NetBeans

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=leenmahdi\&show_icons=true\&theme=tokyonight)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=leenmahdi\&layout=compact\&theme=tokyonight)

---

## 📌 Featured Projects

### 🔗 UniSwap

Peer-to-peer academic platform that allows students to exchange academic services using a points-based system instead of money.

**Tech Stack:** ASP.NET Core, SignalR, JWT, SQL Server, N-Tier Architecture

---

### 💬 Discord Clone

Real-time chat application with authentication, channels, and messaging features.

**Tech Stack:** React.js, Node.js, Socket.io, JWT

---

### 🛒 E-commerce API

RESTful backend system for managing products, authentication, orders, and users.

**Tech Stack:** ASP.NET Core, Entity Framework Core, SQL Server

---

### 🧪 QA Testing Project

Automated testing project using Playwright and TypeScript.

**Tech Stack:** Playwright, TypeScript

---

## 🌱 Currently Learning

* 🚀 Advanced Backend Development
* 🏗️ System Design Basics
* 🧼 Clean Architecture
* 🐍 Django REST Framework
* 🐘 PostgreSQL

---

## 💡 Interests

* 🧠 Backend Development
* 🏗️ Software Engineering
* 🧩 Problem Solving
* 🔌 API Development
* 💬 Real-Time Applications

---

## 📫 Connect With Me

* 🐙 GitHub: [https://github.com/](https://github.com/)
* 💼 LinkedIn: Add your LinkedIn link here
* 📧 Email: Add your email here

---

⭐ *Always learning and building new things.*
